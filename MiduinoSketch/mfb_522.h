...
void randomize_522_seq(ApplicationData& data)
{
    ...
    set_kick_pattern(data.cy_522_pattern);
...
void play_522(ApplicationData& data)
{
    ...
    if (gate(data.bd_522_pattern, data.step) && !data.uiState.kill_low)
    {
        note_on(NOTE_522_BD_LONG, velocity, MIDI_CHANNEL_522, data.storage_522);
    }
    if (gate(data.lo_tom_522_pattern, data.step) && !data.uiState.kill_perc)
    {
        note_on(NOTE_522_LO_TOM, velocity, MIDI_CHANNEL_522, data.storage_522);
    }
    if (gate(data.mi_tom_522_pattern, data.step) && !data.uiState.kill_perc)
    {
        note_on(NOTE_522_HI_TOM, velocity, MIDI_CHANNEL_522, data.storage_522);
    }
    if (gate(data.rs_522_pattern, data.step) && !data.uiState.kill_mid)
    {
        note_on(NOTE_522_RS, velocity, MIDI_CHANNEL_522, data.storage_522);
    }
    if (gate(data.clave_522_pattern, data.step) && !data.uiState.kill_perc)
    {
        note_on(NOTE_522_CLAVE, velocity, MIDI_CHANNEL_522, data.storage_522);
    }
    if (gate(data.clap_522_pattern, data.step) && !data.uiState.kill_mid)
    {
        note_on(NOTE_522_CP_LONG, velocity, MIDI_CHANNEL_522, data.storage_522);
    }
    if (gate(data.hh_522_pattern, data.step) && !data.uiState.kill_high)
    {
        note_on(NOTE_522_HH, velocity, MIDI_CHANNEL_522, data.storage_522);
    }
    if (gate(data.oh_522_pattern, data.step) && !data.uiState.kill_high)
    {
        note_on(NOTE_522_OH, velocity, MIDI_CHANNEL_522, data.storage_522);
    }
    if (gate(data.cy_522_pattern, data.step) && !data.uiState.kill_high)
    {
        note_on(NOTE_522_CYMBAL, velocity, MIDI_CHANNEL_522, data.storage_522);
    }
    if (gate(data.sd_522_pattern, data.step) && !data.uiState.kill_mid)
    {
