...
void HarmonyStruct::randomizeHighPattern(
...
void HarmonyStruct::switchConstChord() {
    ...
    options.erase(
        std::remove(options.begin(), options.end(), const_value),
        options.end());
    options.erase(
        std::remove(options.begin(), options.end(), 0), options.end());
...
void HarmonyStruct::setType(HarmonyType type) { this->type = type; }
