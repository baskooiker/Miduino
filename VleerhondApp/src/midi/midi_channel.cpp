...
MidiChannel::MidiChannel(
    const uint8_t midi_channel, const std::string& port_name)
    : channel(midi_channel), port_name(port_name) {}

MidiChannel::MidiChannel(
    const uint8_t channel, const uint8_t cc_channel,
    const std::string& port_name)
    : channel(channel), cc_channel(cc_channel), port_name(port_name) {}
...
void MidiChannel::printStorage() {
    ...
    for (size_t i = 0; i < this->size; i++) {
        printf(
            "%2d, %3d, %d\n", this->data[i].pitch, this->data[i].length,
            (int)this->data[i].type);
    }
}

void MidiChannel::setPedal(const bool value) { pedal = value; }
bool MidiChannel::getPedal() { return this->pedal; }

// TODO: Remove when obsolete

std::string MidiChannel::getPortName() { return this->port_name; }
